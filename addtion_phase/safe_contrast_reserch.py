...
'''
Date : 2018-1-18
@Author : Amy (本模块程序用于论文提交)
Goal : 6进程并行计算   (对比指标   资源碎片化程度、 容错度)
       1. 对比支持容错的safe_FFDSum_complex与不支持容错的FFDSum_complex
       2. 对比支持容错的safe_FFDSum_complex与支持容错的safe_FFDSum_simple
'''
...
def safe_FFDSum_complex(bins, objects):
    '''
    以docker作为计算负载基础，面向降低集群资源碎片化的新增放置策略
    @param: bins 代表当前系统状态
            objects 代表待放入bin的容器
    @return: 安排好所有objects的集群bins状态
    @note:  此算法进行HMs、VMs打分、并满足HM独立性约束
    '''
    ...
    for x in xrange(len(objects['c_rp'])):
        ...
        while i>0:
            # 对该object(容器)计算active bins（VMs）的得分，仅针对used VMs
            weightedVMBins = safe_weightVMBins_complex(bins, object_CPU, object_MEM, replicas_hms)
            ...
            if len(weightedVMBins) > 0:
                ...
                # 已经解决掉一个object
                replicas_hms.append(hm_suffix)
                i -= 1 
                continue
                
            # 否则，集群已没有能容纳该object（容器）的bin（VM）
            else:
                flag = safe_find_HM_complex(bins, object_CPU, object_MEM, vm_option, replicas_hms)
...
def safe_weightVMBins_complex(bins, object_CPU, object_MEM, replicas_hms):
    '''
    约束：仅对符合HM独立性约束的进行选择
    方式：active VM计分排序引入HM计分项，排序规则负载均以Docker负载之和为基础
    CPU得分sum(reservedCPU/v_rp*100)
    MEM得分sum(reservedMEM/v_rm*100)
    '''
    # print "\n 进入weightVMBins_complex() 方法"
    ...
    # 以active hm:vm的映射开始，其中vm为list
    for h, v in map_h_v.items():
        # 仅对满足HM独立性约束的active hms进行选择
        if h not in replicas_hms:
            # 拿到每个HM实际运行的所有docker产生的负载
            true_hm_cpu = sum([bins['v_p_cost'][0][i] for i in v])
            true_hm_mem = sum([bins['v_m_cost'][0][i] for i in v])
            # 对每个active hm上的active vm进行打分
...
def safe_find_HM_complex(bins, c_rp, c_rm, vm_option, replicas_hms):
    '''
    在集群当前满足HM独立性约束的active HMs中，
    1. 能够容纳该容器(c_rp, c_rm)的最小VM；
    2. 能够容纳该最小VM的所有active HMs，依据实际容器当作负载时资源占比进行打分；
    3. 分高者即满足约束且最为合适的方案，(并对其剩余VM可申请资源全部分配)，若无则需新引入HM。
    '''
    ...
    # 对满足HM独立性约束且能够容纳最小VM的所有active HMs，以容器为实际负载进行资源占比打分
    for h, v in map_h_v.items():
        if h not in replicas_hms:
            reservedCPU = 1.0 - bins['h_p_cost'][0][h]
            reservedMEM = 1.0 - bins['h_m_cost'][0][h]
...
def FFDSum_simple(bins, objects):
    ...
    @param: bins 代表当前系统状态
            objects 代表待放入bin的容器
    @return: 安排好所有objects的集群bins状态
    @note:  此算法仅进行VMs打分，HM选择仅first fit选择满足约束的第一个
    '''
    # print " \n进入 FFDSum_simple() 方法" 
    time0 = time.time()
    ...
    # 情况2： all running vms均无法放入，init_VM产生新VM，First Fit选择第一个可容纳该VM的HM下标
...
def find_HM_simple(bins, v_rp, v_rm, vm_suffix):
    '''
    按照First Fit选择集群中active HMs中第一个能够容纳新建VM的HM下标并更新资源耗费与map_v_h
    '''
    # 找到可满足资源约束的第一个HM
    hm_suffix = -1
...
def safe_FFDSum_simple(bins, objects):
    '''
    @param: bins 代表当前系统状态
            objects 代表待放入bin的容器
    @return: 安排好所有objects的集群bins状态
    ...
    for x in xrange(len(objects['c_rp'])):
        ...
        while i>0:
            # 对该object(容器)计算所有bins（VMs）得分
            weightedVMBins = safe_weightVMBins_simple(bins, object_CPU, object_MEM, replicas_vms)
            ...
            if len(weightedVMBins) > 0:
                ...
                # 否则，需要为该VM找寻可以hosted 的HM,并更新资源
                    ...
                    hm_suffix = safe_find_HM_simple(bins, bins['v_rp'][vm_suffix], bins['v_rm'][vm_suffix], vm_suffix)
            ...
            # 否则，当前集群没有能容纳该object（容器）的bin（VM）
                ...
                # 为该VM找寻HM,并更新HM资源变动及map_v_h
                hm_suffix = safe_find_HM_simple(bins, vm['rp'], vm['rm'], vm_suffix)
...
def safe_weightVMBins_simple(bins, object_CPU, object_MEM, replicas_vms):
    '''
    @note:  以容器放入对VM负载占比计分排序，且必须满足VM独立性约束
    CPU得分sum(reservedCPU/v_rp*100)
    MEM得分sum(reservedMEM/v_rm*100)
    '''
    # print "\n 进入weightVMBins_complex() 方法"
...
def safe_find_HM_simple(bins, v_rp, v_rm, vm_suffix):
    '''
    @note:  此算法对于新建VM都会在active HMs上按照VM放入造成各HM所有维度资源最大占用程度排序选择
    ...
    # print '\n进入 find_HM_complex() 方法'
    # 对集群已有的所有HMs进行打分
    weightedHMBins = safe_weightHMBins_simple(bins, v_rp, v_rm)
...
def safe_weightHMBins_simple(bins, object_CPU, object_MEM):
    '''
    @note:  此算法对于新建VM都会在active HMs上按照VM放入造成各HM所有维度资源最大占用程度排序选择
...
def detect_hm_independce(bins, map_d_s, map_s_d):
    ...
    for service, dockers in map_s_d.items():
        ...
        if nums == 1:
            continue
        # 该服务所有replicas造成的hm-docker映射字典
        d_hms = collections.defaultdict(list)
        for j in dockers:
            d_hms[bins['population'][0][j][-1]].append(j)
        # 若满足独立性，则进行下一个服务检测
        if nums == len(d_hms):
            continue

        # 对服务的所有same hm的容器都进行调整，使得都分布在不同HM上
        while nums != len(d_hms):
            print '服务{}违反约束'.format(service)
            # 逐各replicas所在的hm进行检测与修复
            for hm, dockers in d_hms.items():
                num = len(dockers)
                while num > 1:
                    # 假设不满足HM独立性
                    flag = False
                    # 对该hm上第一个容器从active HMs最大下标者开始依次往前，寻找可容纳该副本vm-hm映射
                    ...
                    for h in d_hms.keys():
                        used_hms.remove(h)
                    # 在满足HM独立性的active hms上修复：降序在各符合独立性约束的HM判断能否放入该服务一个副本
                    for hm2 in sorted(used_hms, reverse=True):
                        dockers_onHM, vms_onHM = find_docker_onHM(bins, hm, 0)
                        # 遍历hm2上各vm与其各容器，选择可容纳replicas或通过容器对掉后可容纳replicas的vm
                        for i in xrange(len(vms_onHM)):
                            vm_suffix = vms_onHM[i]
                            docker_suffix = dockers_onHM[i]
                            # 是否vm_suffix能够容纳replicas
                            ...
                            if reversed_cpu2 >= 0 and reversed_cpu2 >= 0:
                                ...
                                bins['population'][0][origin_docker_suffix] = [vm_suffix, hm2]
                                flag = True
                                d_hms[hm].pop(origin_docker_suffix)
                                d_hms[hm2].append(origin_docker_suffix)
                                dockers.pop(origin_docker_suffix)
                                num -= 1
                                break
                            # 是否与replicas对换后vm_suffix能够容纳replicas
                            ...
                            if reversed_cpu0 >= 0 and reversed_cpu1 >= 0 and reversed_mem0 >= 0 and reversed_mem1 >=0:
                                ...
                                bins['population'][0][origin_docker_suffix] = [vm_suffix, hm2]
                                bins['population'][0][docker_suffix] = [origin_vm_suffix, hm]
                    ...
                    # 若满足HM独立性约束的active hms均无法用于修复，则新建VM并选择HM放入          
                    if not flag:
                        # 在满足独立性约束的active hms上选择能容纳replicas的最小VM及其可放入HM
                        second_flag = find_HM_complex(bins, object_CPU, object_MEM, vm_option, d_hms.keys())
                        # 代表active HMs中有HM可以容纳能够放入该容器的最小VM
                        if second_flag:
                            hm_suffix, min_vm = second_flag
                            vm_suffix = len(bins['v_p_cost'][0])
                            ...
                            flag = True
                            d_hms[hm].pop(origin_docker_suffix)
                            d_hms[hm_suffix].append(origin_docker_suffix)
                            dockers.pop(origin_docker_suffix)
                            num -= 1
...
def detect_vm_independce(bins, map_d_s, map_s_d):
    '''
    检测bins集群VM为独立性约束并对违约进行修复
    @para: map_d_s, map_s_d 分别是docker-service与service-docker映射
    @func: 在bins中通过map_s_d检测各服务所有replicas实际所在VMs，对于all replicas same VM情况，
            则至少变换一个replica使其与集群中最大下标VM上某容器交换位置，直至检查完所有的service并返回修复独立性之后的bins
    '''
    # 遍历所有容器service
    for service, dockers in map_s_d.items():
        # 各服务所需容器配置及replicas数量
        ...
        if nums == 1:
            continue
        # 该服务所有replicas造成的vm-docker映射字典
        d_vms = collections.defaultdict(list)
        for j in dockers:
            d_vms[bins['population'][0][j][0]].append(j)
        # 若满足独立性，则进行下一个服务检测
        if nums == len(d_vms):
            continue

        # 对服务的所有same vm的容器都进行调整，使得都分布在不同VM上
        while nums != len(d_vms):
            # 不满足独立性需要修复
            for vm, dockers in d_vms.items():
                num = len(dockers)
                while num > 1:
                    # 独立性修复
                    flag = False
                    # 对该VM上第一个容器从active VMs最大下标者开始依次往前，寻找可与该容器对调换位置的容器
                    origin_docker_suffix = dockers[0]
                    # 满足独立性约束的active vms
                    used_vms = bins['map_v_h'].keys()
                    for v in d_vms.keys():
                        used_vms.remove(v)
                    # 在符合VM独立性的active vms进行修复：降序在各符合独立性约束的VM判断能够放入该服务一个副本
                    for vm2 in sorted(used_vms, reverse=True):
                        # 能否直接放入该服务副本
                        reversed_cpu2 = bins['v_rp'][vm2] - (bins['v_p_cost'][0][vm2] + object_CPU)
                        reversed_mem2 = bins['v_rm'][vm2] - (bins['v_m_cost'][0][vm2] + object_MEM)
                        if reversed_cpu2 >= 0 and reversed_cpu2 >= 0:
                            bins['v_p_cost'][0][vm2] += object_CPU
                            bins['v_m_cost'][0][vm2] += object_MEM
                            bins['v_p_cost'][0][vm] -= object_CPU
                            bins['v_m_cost'][0][vm] -= object_MEM
                            bins['population'][0][origin_docker_suffix] = [vm2, bins['map_v_h'][vm2]]
                            flag = True
                            d_vms[vm].pop(origin_docker_suffix)
                            d_vms[vm2].append(origin_docker_suffix)
                            dockers.pop(origin_docker_suffix)
                            num -= 1
                            break
                        # 无法直接放入该副本则通过该vm2容器与副本对掉能否放入
                        dockers_onVM = find_docker_onHM(bins, vm2, 1)
                        for d in dockers_onVM:
                            # vm2换出原docker换入replica之后的资源
                            reversed_cpu1 = bins['v_rp'][vm2] - (bins['v_p_cost'][0][vm2] - bins['c_rp'][d] + object_CPU)
                            reversed_mem1 = bins['v_rm'][vm2] - (bins['v_m_cost'][0][vm2] - bins['c_rm'][d] + object_MEM)
                            # vm换入新容器换出replica后的资源
                            reversed_cpu0 = bins['v_rp'][vm] - (bins['v_p_cost'][0][vm] + bins['c_rp'][d] - object_CPU)
                            reversed_mem0 = bins['v_rm'][vm] - (bins['v_m_cost'][0][vm] + bins['c_rm'][d] - object_MEM)
                            # 若对换后满足资源约束，则更新集群
                            if reversed_cpu0 >= 0 and reversed_cpu1 >= 0 and reversed_mem0 >= 0 and reversed_mem1 >=0:
                                bins['v_p_cost'][0][vm2] += (object_CPU - bins['c_rp'][d])
                                bins['v_m_cost'][0][vm2] += (object_MEM - bins['c_rm'][d])
                                bins['v_p_cost'][0][vm] += (bins['c_rp'][d] - object_CPU)
                                bins['v_m_cost'][0][vm] += (bins['c_rm'][d] - object_MEM)
                                bins['population'][0][origin_docker_suffix] = [vm2, bins['map_v_h'][vm2]]
                                bins['population'][0][d] = [vm, bins['map_v_h'][vm]]
                                # map_v_h由于是对掉并不会产生新的拓扑变化，故不用更新
                                flag = True
                                d_vms[vm].pop(origin_docker_suffix)
                                d_vms[vm2].append(origin_docker_suffix)
                                ...
                                num -= 1
                                break
                        # 若以flag = True跳出循环则修复了一个副本,则退出本次外for修复
                        if flag:
                            break
                            
                    # 若满足VM独立性约束的active vms均无法用于修复，则新建VM并选择HM放入          
                    if not flag:
                        vm = create_VM_random(object_CPU, object_MEM, vm_option)
                        vm_suffix = len(bins['v_p_cost'][0])
                        ...
                        # 更新‘population’、‘map_v_h’
                        bins['population'][origin_docker_suffix] = [vm_suffix, hm_suffix]
                        flag = True
                        d_vms[vm].pop(origin_docker_suffix)
                        d_vms[vm_suffix].append(origin_docker_suffix)
                        dockers.pop(origin_docker_suffix)
                        num -= 1     
    return bins
...
def detect_FFDSum_complex(bins, addtion0, map_d_s, map_s_d):
    bins = FFDSum_complex(bins, addtion0)
    # 对结果进行容错调整
    bins = detect_hm_independce(bins, map_d_s, map_s_d)
    return bins

def main_controller(function_str, bins, addtion0, map_d_s, map_s_d, func_handler, return_dict, flag):
    '''
    2017-12-20 23:00 使用多进程共享变量与多进程实现4种方式并行计算
    function_str为不同进行在共享变量字典中的key值
    使用函数句柄实现不同方法的串行计算逻辑
    flag == 0 即为安全性容错
    '''
    # 初始放置方案的计算
...
def create_addtion0(p, scale, last_docker_suffix, last_service_suffix):
    '''
    @params: p相关系数， scale本次增加的容器数量，
             last_docker_suffix 集群已有容器总量最大下标， last_docker_suffix 集群已有服务最大下标
    @return: 各服务的尺寸规模即addtion0,map_d_s与map_s_d
    '''
    map_d_s = {}
    suffix = -1  # 记录上次安排完所属服务的容器
    count = last_service_suffix + 1    # 记录服务下标
    flag = scale
    while flag > 0:
        # 模拟生成需要随机数量replicas支持的服务
        x = random.randint(0, 5)
        for i in xrange(x):
            if len(map_d_s) < scale :
                map_d_s[i+suffix+1+last_docker_suffix+1] = count
        flag -= x
        count += 1
        if x != 0:
            suffix = len(map_d_s) - 1
    ...
    for key, value in map_d_s.items():
        map_s_d[value].append(key)
    service_scale = len(map_s_d.keys())
    # 调用init模块create_addtion方法创建各服务容器cpu、mem规格
    addtion0 = create_addtion(p, service_scale)
    # 更新addtion0字典中服务对应的replicas数量
    addtion0['replicas'] = [len(map_s_d[i]) for i in sorted(map_s_d.keys())]
    return addtion0, map_d_s, map_s_d
...
if __name__ == '__main__':
    ...
    # 强肯定关联程度容器类型
    init_popu0 = main_init(3, 1.0)          # 用于safe_complex算法
    ...
    map_d_s0, map_s_d0 = docker2service(3)   # 初始d-s与时-d分布,后期用于记录addtion0的服务矩阵
    map_d_s1 = copy.deepcopy(map_d_s0)       # 后期用于记录addtion1的服务矩阵
    map_s_d1 = copy.deepcopy(map_s_d0)       # 后期用于记录addtion1的服务矩阵
    ...
    # 3. 模拟多批量新增场景,scale为每次共增加的容器数量，不是服务数量
    for scale in cycle:
        ...
        last_docker_suffix = len(init_popu0['c_rp'])
        last_service_suffix0 = max(map_s_d0)  # for addtion0
        last_service_suffix1 = max(map_s_d1)  # for addtion1
        # 无关联程度类型容器
        # addtion0, add_d_s0, add_s_d0 = create_addtion0(0.02, scale, last_docker_suffix, last_service_suffix0)
        # addtion1, add_d_s1, add_s_d1 = create_addtion0(0.02, scale, last_docker_suffix, last_service_suffix1)
        
        # 强否定关联程度类型容器
        # addtion0, add_d_s0, add_s_d0 = create_addtion0(-0.75, scale, last_docker_suffix, last_service_suffix0)
        # addtion1, add_d_s1, add_s_d1 = create_addtion0(-0.75, scale, last_docker_suffix, last_service_suffix1)
        
        # 强肯定关联程度类型容器
        addtion0, add_d_s0, add_s_d0 = create_addtion0(1.0, scale, last_docker_suffix, last_service_suffix0)
        addtion1, add_d_s1, add_s_d1 = create_addtion0(1.0, scale, last_docker_suffix, last_service_suffix1)
        # 对每次新增都持续增加d-s与s-d映射
        map_d_s0.update(add_d_s0)
        map_s_d0.update(add_s_d0)
        map_d_s1.update(add_d_s1)
        map_s_d0.update(add_s_d1)
        ...
        for gen in xrange(Gen):
            if gen == 0:
                # safe_complex算法
                p = multiprocessing.Process(target=main_controller, args=(0, init_popu0, addtion0, map_d_s0, map_s_d0, safe_FFDSum_complex, return_dict, 0))
                jobs.append(p)
                p.start()
            elif gen == 1:
                # simple 算法
                p = multiprocessing.Process(target=main_controller, args=(1, init_popu1, addtion0, map_d_s0, map_s_d0, safe_FFDSum_simple, return_dict, -1))
                jobs.append(p)
                p.start()
            elif gen == 2:
                # safe_complex算法
                p = multiprocessing.Process(target=main_controller, args=(2, init_popu2, addtion1, map_d_s1, map_s_d1, safe_FFDSum_complex, return_dict, 0))
                jobs.append(p)
                p.start()
            elif gen == 3:
                # simple算法
                p = multiprocessing.Process(target=main_controller, args=(3, init_popu3, addtion1, map_d_s1, map_s_d1, safe_FFDSum_simple, return_dict, -1))
                jobs.append(p)
                p.start()
            elif gen == 4:
                # complex算法
                p = multiprocessing.Process(target=main_controller, args=(4, init_popu4, addtion0, map_d_s0, map_s_d0, FFDSum_complex, return_dict, 1))
                jobs.append(p)
                p.start()
            elif gen == 5:
                # complex算法
                p = multiprocessing.Process(target=main_controller, args=(5, init_popu5, addtion1, map_d_s1, map_s_d1, FFDSum_complex, return_dict, 1))
                jobs.append(p)
                p.start()
