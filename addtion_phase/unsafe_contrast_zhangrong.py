...
Date : 2018-1-18
@Author : Amy
Goal : 非容错的工业实现与本研究1,1的对比（最终将对比结果输出为excel文件）
...
def createJSON(data, addtion_scale, cost0, cost1, cost2, cost3):
    '''
    ...
    data['simple_hms'].append(cost0)
    data['complex_hms'].append(cost1)
    data['simple_concentration'].append(cost2)
    data['complex_concentration'].append(cost3)
    return data
...
if __name__ == '__main__':
    ...
    for p_relation in [0.75, 0.02, -0.75]:
        ...
        # 2. 产生新初始集群与新增容器的服务数量
        init_popu0 = main_init(20, p_relation)      # 用于simple算法
        ...
        for scale in cycle:
            avg_simple_hms = 0
            avg_simple_concentration = 0.0
            avg_complex_hms = 0
            avg_complex_concentration = 0.0

            # 为了体现随机性，分别为各进程生成每批的新增服务及其副本
            addtion0 = create_addtion(p_relation, scale)
            addtion1 = create_addtion(p_relation, scale)
            ...
            # 计算迭代gen代的平均值,并写入文件
            avg_simple_hms += (return_dict[0][0] + return_dict[2][0])
            avg_simple_concentration += (return_dict[0][1] + return_dict[2][1])
            avg_complex_hms += (return_dict[1][0] + return_dict[3][0])
            avg_complex_concentration += (return_dict[1][1] +return_dict[3][1])
            avg_scale /= Gen/2
            avg_simple_hms /= Gen/2
            avg_simple_concentration /= Gen/2
            avg_complex_hms /= Gen/2
            avg_complex_concentration /= Gen/2
            data = createJSON(data, avg_scale, avg_simple_hms, avg_complex_hms, avg_simple_concentration, avg_complex_concentration)
        ## 4. 记录data用于前端数据可视化
            with open('.//viz//unsafe-contrast-multiprocess4-{}.json'.format(p_relation),'a') as f:
                f.flush()
                json.dump(data, f, indent=2)

        # 5. 程序循环计算结束并记录json文件后，将最终的字典data写入excel文件
        # 创建excel工作表
        ...
        worksheet.write(0, 1, label='industrialization used hms')
        worksheet.write(0, 2, label='my reserch used hms')
        worksheet.write(0, 3, label='industrialization fragment')
        worksheet.write(0, 4, label='my reserch fragment')

        val1, val2, val3, val4, val5 = 1, 1, 1, 1, 1
        # 将data字典写入excel中
        for key, value in data.items():
            ...
            if key == "scale":
                ...
                for s in value:
                    ...
                    print "已经写入第1列"
                    val2 += 1
            elif key == "complex_hms":
                for s in value:
                    worksheet.write(val3, 2, s)
                    print "已经写入第2列"
                    val3 += 1
            elif key == "simple_concentration":
                for s in value:
                    worksheet.write(val4, 3, s)
                    print "已经写入第3列"
                    val4 += 1
            elif key == "complex_concentration":
                for s in value:
        ...
        # 保存excel文件
        workbook.save('.//viz//unsafe-contrast-multiprocess4-{}.xls'.format(p_relation))